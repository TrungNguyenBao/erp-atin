"""SQL view: accounts receivable aging by project and customer."""

from odoo import fields, models, tools


class ProjectReceivableReport(models.Model):
    _name = 'project.receivable.report'
    _description = 'Project Accounts Receivable Report'
    _auto = False
    ...
    amount_total = fields.Float(string='Invoice Total', readonly=True)
    ...
    def init(self):
        """Create SQL view for receivable aging by project."""
        ...
        self.env.cr.execute(f"""
            CREATE OR REPLACE VIEW {self._table} AS (
                ...
                -- Link invoice -> project via invoice_origin -> SO -> SOL -> project
                LEFT JOIN LATERAL (
                    SELECT pp2.id
                    FROM sale_order so2
                    ...
                    AND am.invoice_origin LIKE '%%' || so2.name || '%%'
                    LIMIT 1
                ) pp ON true
